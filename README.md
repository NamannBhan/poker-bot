...
Share the Vercel link — anyone can play, no setup needed. Your laptop doesn't need to be on.

**Railway free tier:** ~$5 credit/month. Light usage costs $1–3/month. Add a card to avoid surprise pauses.

---

## Known Quirks

- `treys` uses **0-based class integers** for hand ranks (0 = Royal Flush, 9 = High Card) — not 1–10. The original code had this wrong; it's fixed.
- `T` in card display = **Ten** (standard poker notation, not a bug)
- `__pycache__` folders are auto-generated by Python, safe to delete or `.gitignore`
